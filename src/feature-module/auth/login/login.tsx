...
const Login = () => {
  ...
  const loginUser = async (inputs: any) => {
    try {
      setLoading(true);
      const res = await api.post("/api/user/login", inputs);
      console.log(res.data.user.RoleId,"res.data")
      if (res.data.user.RoleId == 1) {
        ...
        toast.error("You are not eligible to  access this account");
      }
    } catch (error) {
      setLoading(false);
