...
interface AttendanceRecord {
  ID: number;
  note: string;
  StudentID: string; // This should be the unique student identifier
  StudentName: string;
  StudentTag: string;
  StudentNo: string;
  IsPresent: number;
...
const StudentDetail: React.FC = () => {
  ...
  const [modalStatus, setModalStatus] = useState("1"); // "1" for Present, "0" for Absent
  ...
  const transformToMonthView = (
    ...
    // Calculate the 3 months: April, May, June (if date is June)
    ...
    months.forEach(({ month, year, label, days }, index) => {
      ...
      // Flag to capture first matching record's ID
      ...
      records.forEach((item:any) => {
        ...
        if (itemMonth === month && itemYear === year) {
          ...
          // Assign note if available
          if (item.note) row.note = item.note;

          // Assign id from first matched record
  ...
  const getUserDetails = async (
    ...
    try {
      ...
      const res = await api.post("/api/mssql-procedure/execute", {
        procedureName: "StudentAttendenceGetById",
        parameters: [
          { name: "StudentID", type: "Int", value: id },
  ...
  useEffect(() => {
    getUserDetails(timeRange, selectedDate);
  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      const res = await api.post("/api/mssql-procedure/execute", {
        ...
        parameters: [
          { name: "ID", type: "Int", value: ids },
          { name: "StudentID", type: "Int", value:id},
          { name: "IsPresent", type: "Int", value: status },
          { name: "note", type: "VarChar", value: note },
          { name: "type", type: "VarChar", value: "manual" },
                    ...
                    <div className="d-flex align-items-center justify-content-between flex-wrap">
                      <div className="d-flex align-items-center">
                        ...
                        <div>
                          ...
                          <div className="d-flex flex-wrap align-items-center mt-1">
                            ...
                            <small className="text-muted me-3">
                              <i className="fas fa-id-card me-1"></i> Center:{" "}
                              {teacher.CenterName}
                            </small>
                            <small className="text-muted me-3">
                      ...
                      <div className="d-flex align-items-center mt-2 mt-sm-0">
                        ...
                        <div className="text-center">
                          <h6 className="mb-0 fw-semibold">
                            {teacher.PresentCount || 0}/{teacher.AbsentCount || 0}
                          </h6>
