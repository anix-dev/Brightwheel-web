...
const AddClass = () => {
  ...
  const CreateClass = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.post("/api/mssql-procedure/execute", {
        procedureName: "ClassesInsert",
        parameters: [
          { name: "CenterID", type: "Int", value: id?.trim() },
      ...
      if(res?.data?.record[0].message== 0){
        return notifySuccess("Class already exist for this center")
      }
      else  {
  ...
  const singleCenterDetails = async () => {
    try {
      ...
      console.log(res.data.record,"CENTER DETAILSSSS");

      if (res.data.success) {
        setServices(res.data.record[0]?.ServiceID || res.data.record[0]?.ServiceIDs);
      }
    } catch (error: any) {
  ...
  const getData = async () => {
    try {
      const res = await api.post("/api/mssql-procedure/execute", {
        ...
        parameters: [{ name: "ID", type: "Int", value:classId }],
      });
    ...
    <div className="page-wrapper">
      <div className="content content-two">
        <div className="">
          ...
          >
            <h2 className="mb-1">{classId?'Edit Class':'Add Class'}</h2>
          </div>
